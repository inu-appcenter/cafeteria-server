...
export default {
  ...
  async checkDiscountAvailability(request, h) {
    ...
    const transaction = resolve(LegacyTransactionConverter).convert({barcode, code/* effectively a cafeCode */});
    const token = menu; /* token does NOT belong to a transaction. */

    const result = await resolve(ValidateDiscountTransaction).run({transaction, token});
  ...
  async commitDiscountTransaction(request, h) {
    ...
    const transaction = resolve(LegacyTransactionConverter).convert({barcode, code, menu});
    const confirm = payment === 'Y';

    const result = await resolve(CommitDiscountTransaction).run({transaction, confirm});
