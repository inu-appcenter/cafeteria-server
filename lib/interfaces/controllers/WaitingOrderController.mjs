...
export default {
  ...
  async pushNumber(request, h) {
    ...
    if (waitingOrderFound['done']) {
      return Boom.notFound(`Order(number: ${num}, code: ${code}) is already marked done.`);
    }

    await resolve(NotifyOrderReady).run({
      waitingOrder: waitingOrderFound,
    });

    await resolve(MarkOrderDone).run({
      waitingOrder: waitingOrderFound,
    });
