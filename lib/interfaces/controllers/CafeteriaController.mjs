...
import CafeteriaCommentSerializer from '../serializers/CafeteriaCommentSerializer.mjs';
...
export default {
  ...
  async getMenus(request) {
    const {cornerId, date, split} = request.query;
    ...
    return resolve(MenuSerializer).serialize(result, {split: !!split});
  },

  async getComments(request) {
    const {cafeteriaId} = request.params;

    const result = await resolve(GetCafeteriaComment).run({cafeteriaId});

    if (!result) {
      return Boom.notFound();
    }

    return resolve(CafeteriaCommentSerializer).serialize(result);
  },
