...
function parse(rawObject, cornerMenuKeys) {
  ...
  let n = 1;
  let rawCorners = rawObject[getFieldName(n++)];
  while (rawCorners) {
    for (const rawCorner of rawCorners) {
      ...
      const found = cornerMenuKeys.find((corner) =>
        corner.TYPE1 === parseInt(rawCorner.TYPE1) && /* must be a number */
      ...
      if (found && rawCorner.MENU) {
        const splited = split(rawCorner.MENU);
    ...
    rawCorners = rawObject[getFieldName(n++)];
  }
