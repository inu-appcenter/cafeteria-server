...
class DirectMenuConverter extends BaseConverter {
  ...
  async onConvert({rawHtml, cafeteria, corners}) {
    ...
    const toDoWithCafeteriaAndCornerNames = async (thisCafeteria, thisCorner) => {
      ...
      const asDomainObject = parsed.map(({foods, price, calorie}) => new Menu({foods, price, calorie, cornerId: thisCorner.id}));
      const filtered = asDomainObject.filter(({foods}) => foods.length > 0);
