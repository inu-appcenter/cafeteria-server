 ...
 * @param	{Object} matchKeys array of id, cafeteriaId, type1, and type2.
 * @return	{array}	array of {@link Menu}.
 */
function _parse(rawObject, cornerMenuKey) {
	if (!rawObject) {
		throw new Error("Invalid rawObject!");
	}
	if (!cornerMenuKey) {
		throw new Error("Invalid cornerMenuKey!");
	}
	...
	while (corners) {
		for (var corner of corners) {
			...
			const found = cornerMenuKey.find(c =>
			...
			if (found && corner.MENU) {
				const splited = _split(corner.MENU);
				for (var menu of splited) {
					result.push(new Menu(found.id, menu.foods, menu.price, menu.calorie));
				}
			}
		}
...
module.exports = class {
	...
	convert(rawObject, cornerMenuKey) {
		return _parse(rawObject, cornerMenuKey);
	}
