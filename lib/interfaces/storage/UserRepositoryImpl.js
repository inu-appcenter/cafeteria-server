...
module.exports = class {
	...
	async tryLoginWithIdAndPassword(id, password) {
		const pwEncrypted = encrypt(password, cryptoConfig.key);
		const loginResponse = await fetch.post(loginConfig.url, { sno: id, pw: pwEncrypted });

		if (loginResponse === loginConfig.success) {
			...
			return UserRepository.LOGIN_OK;
		} else if (loginResponse === loginConfig.fail) {
