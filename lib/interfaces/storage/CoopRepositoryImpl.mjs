...
class CoopRepositoryImpl extends CoopRepository {
  ...
  async _request(url, method, searchParams, data) {
    ...
    logger.verbose(`Send ${method.toUpperCase()} request to '${url}', with data '${JSON.stringify(data)}'`);
  ...
  getWeekDiff(then, now=new Date()) {
    ...
    return dateUtil.getWeek(then, 1) - dateUtil.getWeek(now, 1);
  }
}
