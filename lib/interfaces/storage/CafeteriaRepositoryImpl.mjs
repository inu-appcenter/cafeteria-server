...
import CafeteriaComment from '../../domain/entities/CafeteriaComment.mjs';
...
class CafeteriaRepositoryImpl extends CafeteriaRepository {
  constructor({db, remoteDataSource, menuConverter}) {
    ...
    this.commentModel = this.db.model('cafeteria_comment');
  ...
  async getAllComments() {
    const seqComments = await this.commentModel.findAll();

    return seqComments.map((seqComment) => this._seqCommentToComment(seqComment));
  }

  async getCommentByCafeteriaId(cafeteriaId) {
    if (!cafeteriaId) {
      return null;
    }

    const seqComment = await this.commentModel.findByPk(cafeteriaId);

    if (!seqComment) {
      return null;
    }

    return this._seqCommentToComment(seqComment);
  }

  _seqCommentToComment(seqComment) {
    return new CafeteriaComment({
      comment: seqComment.comment,
      cafeteriaId: seqComment.cafeteria_id,
    });
  }
