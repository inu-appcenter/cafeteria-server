...
class CafeteriaRepositoryImpl extends CafeteriaRepository {
  ...
  async getCafeteriaIdByPosNumber(posNumber) {
    ...
    const found = allKioskNumbers.find((it) => posNumber in it.kioskNumbers);
  ...
  async _getAllCafeteriaKioskNumbers() {
    ...
    return all.map((seqKioskNumbers) => new CafeteriaKioskNumbers({
      kioskNumbers: this._parseKioskNumbersFromCommaJoinedString(seqKioskNumbers.kiosk_numbers),
      cafeteriaId: seqKioskNumbers.cafeteria_id,
    }));
  }

  _parseKioskNumbersFromCommaJoinedString(kioskNumbers) {
    if (!kioskNumbers) {
      return [];
    }
