...
class DiscountTransactionValidatorImpl extends DiscountTransactionValidator {
  ...
  async requestShouldBeInMealTime(cafeteriaId, mealType/* 0 or 1 or 2 */) {
    ...
    if (!mealTypeSupportedByCafeteria) {
      logger.warn(`mealType ${mealType} is not supported by cafeteria(supports ${validationParams.availableMealTypes})`);
      return false;
    }
    ...
    const mealTypeToTimeRange = (mt) => {
      ...
      return validationParams.timeRanges[mealTypeName];
    };

    const timeRange = mealTypeToTimeRange(mealType);
    const nowInTimeRange = timeUtil.isTimeInRange(timeRange);

    if (!nowInTimeRange) {
      logger.warn(`current time ${new Date()} is not in time range ${timeRange}.`);
      return false;
    }
