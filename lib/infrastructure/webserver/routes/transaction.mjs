 ...
 * so they cannot be changed or upgraded easily.
 *******************************************************************/

/**
 * A Deep Dive into The Old API Implementation.
 *
 ...
 * this server writes the status change to its DB.
...
const discountAvailabilityCheckQueryModel = Joi.object({
  barcode: Joi.string().required().description('사용자가 태그한 바코드'),
  code: Joi.number().required().description('사용자가 태그한 식당 코드. cafeteriaId와 호환 안됨.'),
  menu: Joi.string().required().description('식당에 부여된 고유 토큰'),
...
const commitDiscountTransactionQueryModel = Joi.object({
  ...
  code: Joi.number().required().description('사용자가 태그한 식당 코드. cafeteriaId와 호환 안됨.'),
  menu: Joi.string().required().description('식당에 부여된 고유 토큰'),
  payment: Joi.string().required().description('트랜잭션 기록/삭제 여부'),
