...
const DiscountTransactionController = require('@interfaces/controllers/DiscountTransactionController');
 ...
 */
const discountAvailibilityCheckQueryModel = Joi.object({
	barcode: Joi.string().description('사용자가 태그한 바코드'),
	code: Joi.number('사용자가 태그한 식당 코드. cafeteriaId와 호환 안됨.'),
	menu: Joi.string('식당에 부여된 고유 토큰')
}).label('isBarcode 쿼리 모델');

const discountAvailibilityCheckUsualResponseModel = Joi.object({
	message: Joi.string().description('요청이 별 탈 없이 처리되었으면 SUCCESS'),
	activated: Joi.number('할인받을 수 있다면 1, 아니면 0')
}).label('isBarcode 통상 응답 모델');

const discountAvailibilityCheckUnusualResponseModel = Joi.object({
...
const commitDiscountTransactionQueryModel = Joi.object({
	...
	code: Joi.number('사용자가 태그한 식당 코드. cafeteriaId와 호환 안됨.'),
	menu: Joi.string('식당에 부여된 고유 토큰'),
	payment: Joi.string('트랜잭션 기록/삭제 여부');
 	...
 	register: async (server) => {
 		...
 		server.route([
			 ...
			 */
			{
				method: 'GET',
				path: '/isBarcode',
				handler: DiscountTransactionController.checkDiscountAvailibility,
				options: {
					description: '[Private] 할인을 제공할 수 있는지 여부를 판단합니다.',
					notes: ['할인 적용 시간, 바코드 활성화 여부, 최근 할인 기록 등을 종합하여 할인 가능 여부를 판단합니다.'],
					tags: ['api', 'transaction'],
					validate: {
						query: discountAvailibilityCheckQueryModel
					},
					response: {
						status: {
							200: discountAvailibilityCheckUsualResponseModel,
							400: discountAvailibilityCheckUnusualResponseModel,
							500: BoomModel
						}
					}
				}
