...
const queryModel = Joi.object({
  date: Joi.string().description('가져오고자 하는 Menu들의 날짜'),
  cornerId: Joi.number().description('가져오고자 하는 Menu들이 속한 Corner의 id. 특정 코너의 식단만 가져오고 싶을 때에 유용합니다.'),
  split: Joi.boolean().optional().description('메뉴를 분리하여 배열로 받을 것인지 여부(기본 false)'),
}).label('Menu 쿼리 모델');
