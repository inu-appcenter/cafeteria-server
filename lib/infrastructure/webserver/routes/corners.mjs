...
const responseModel = Joi.object({
  ...
  'display-name': Joi.string().allow('', null).description('Corner 표시 이름'),
  'available-at': Joi.number().description('Corner의 식사 제공 시간대'),
  'cafeteria-id': Joi.number().description('Corner가 속한 Cafeteria의 id'),
}).label('Corner 모델');

const allQueryModel = Joi.object({
  cafeteriaId: Joi.number().description('해당 Corner가 속한 Cafeteria의 id'),
}).label('Corner 쿼리 모델');

const idParamModel = Joi.object({
  id: Joi.number().required().description('Corner의 id.'),
