...
const responseModel = Joi.object({
  ...
  'created-at': Joi.number().description('공지 시각'),
}).label('Notice 모델');
...
const paramModel = Joi.object({
  id: Joi.alternatives().try(
    Joi.string(),
    Joi.number(),
  ).required().description('공지 번호 또는 latest'),
});

const queryModel = Joi.object({
  os: Joi.string().optional().description('기기 운영체제'),
  version: Joi.string().optional().description('설치된 앱 버전'),
}).label('공지 확인 쿼리 모델');
