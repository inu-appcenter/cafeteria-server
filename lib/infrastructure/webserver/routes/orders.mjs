...
const waitingOrderFetchQueryModel = Joi.object({
  deviceIdentifier: Joi.string().description('대기주문을 등록한 기기의 식별자'),
...
const waitingOrderPostModel = Joi.object({
  number: Joi.number().description('주문 번호'),
  cafeteriaId: Joi.number().description('주문한 식당의 id'),
  deviceIdentifier: Joi.string().description('기기의 식별자'),
}).label('WaitingOrder 생성 입력 모델');

const waitingOrderDeleteModel = Joi.object({
  waitingOrderId: Joi.number().description('삭제할 주문 번호'),
}).label('WaitingOrder 삭제 입력 모델');

const orderReadyQueryModel = Joi.object({
  number: Joi.number().description('주문 번호'),
  code: Joi.number().description('레거시 cafe code'),
...
const deleteOrder = {
  ...
  options: {
    ...
    tags: ['api', 'orders'],
    validate: {
      payload: waitingOrderDeleteModel,
    },
    response: {
      status: {
        201: undefined,
