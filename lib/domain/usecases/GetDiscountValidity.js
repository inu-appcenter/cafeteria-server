...
class GetDiscountValidity extends UseCase {
  static USUAL_SUCCESS = 0
  static USUAL_FAIL = 1
  static UNUSUAL_NO_BARCODE = 2
  static UNUSUAL_WRONG_PARAM = 3
  ...
  async onExecute({transaction, token}) {
    ...
    if (!isInMealTime) {
      return GetDiscountValidity.USUAL_FAIL;
    }

    const userExists = await this.transactionValidator
        .userExists(userId);
    if (!userExists) {
      return GetDiscountValidity.UNUSUAL_NO_BARCODE; /* no barcode = no user */
    }
    ...
    if (!tokenIsValid) {
      return GetDiscountValidity.UNUSUAL_WRONG_PARAM;
    }

    return GetDiscountValidity.USUAL_SUCCESS;
  }
}

export default GetDiscountValidity;
