...
module.exports = async (
  {transaction/* technically a transaction request */, token},
  {validator},
) => {
  ...
  const barcodeIsActive = await validator.isBarcodeActive(userId, 10/* available for 10 mins */);
  if (!barcodeIsActive) {
    return returnCodes.USUAL_FAIL;
  }
  ...
  const isNotFrequent = await validator.barcodeNotUsedRecently(userId, 15/* interval 15 secs */);
  if (!isNotFrequent) {
    return returnCodes.USUAL_FAIL;
  }
