...
class Login extends UseCase {
  ...
  async onExecute({token, id, password}) {
    let result;

    if (token) {
      result = await this.userRepository.tryLoginWithToken(token);
    } else if (id && password) {
    ...
    if (!err) {
      ...
      const newBarcode = this.barcodeTransformer.generateBarcodeWithId(userId);

      // Set the barcode to the user.
      await this.userRepository.setBarcode(userId, newBarcode);
    }
