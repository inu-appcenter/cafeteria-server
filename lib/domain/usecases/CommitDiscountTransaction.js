...
import CommitResults from '#domain/entities/DiscountCommitResults';
  ...
  async onExecute({transaction, confirm}) {
    if (confirm) {
      ...
      if (firstToday) {
        ...
        if (writeResult) {
          return CommitResults.SUCCESS;
        } else {
          return CommitResults.FAIL;
        }
      } else {
        // Not first today.
        return CommitResults.ALREADY_DISCOUNTED;
      }
    } else {
