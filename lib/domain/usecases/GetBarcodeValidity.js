...
module.exports = async ({ transaction/* technically a transaction request */ }, { validator }) => {
	...
	const isInMealTime = await validator.isInMealTime(transaction.mealType);
	if (!isInMealTime) {
		return returnCodes.USUAL_FAIL;
	}

	const userExists = await validator.userExists(transaction.userId);
	if (!userExists) {
		return returnCodes.UNUSUAL_NO_BARCODE; /* no barcode = no user */
	}

	const barcodeIsActive = await validator.isBarcodeActive(transaction.userId);
	if (!barcodeIsActive) {
		return returnCodes.USUAL_FAIL;
	}

	const isFirstToday = await validator.isFirstToday(transaction.userId);
	if (!isFirstToday) {
		return returnCodes.USUAL_FAIL;
	}

	const isNotFrequent = await validator.barcodeNotUsedRecently(transaction.userId);
	if (!isNotFrequent) {
		return returnCodes.USUAL_FAIL;
	}

	const tokenIsValid = await validator.isTokenValid(transaction.cafeteriaId, transaction.token);
	if (!tokenIsValid) {
