...
const returnCodes = {
	...
	UNUSUAL_INTERNAL_ERROR: 4,
};

module.exports = async (options, { validator }) => {

	/**
	 * Parameters are valid.
	 * They are validated before in router.
	 */

	 // a DiscountTransaction instance
	const requested = options.transaction;

	const isInMealTime = await validator.isInMealTime(requested.mealType);
	if (!isInMealTime) {
		return returnCodes.USUAL_FAIL;
	}

	const userExists = await validator.userExists(request.userId);
	if (!userExists) {
		return returnCodes.UNUSUAL_NO_BARCODE; /* no barcode = no user */
	}

	const barcodeIsActive = await validator.isBarcodeActive(request.userId);
	if (!barcodeIsActive) {
		return returnCodes.USUAL_FAIL;
	}

	const isFirstToday = await validator.isFirstToday(request.userId);
	if (!isFirstToday) {
		return returnCodes.USUAL_FAIL;
	}

	const isNotFrequent = await validator.barcodeNotUsedRecently(request.userId);
	if (!isNotFrequent) {
		return returnCodes.USUAL_FAIL;
	}

	const tokenIsValid = await validator.isTokenValid(requested.cafeteriaId, requested.token);
	if (!tokenIsValid) {
