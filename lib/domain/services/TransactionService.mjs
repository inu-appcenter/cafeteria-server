...
class TransactionService {
  ...
  async _proceedCommittingTransaction(transaction) {
    ...
    const firstToday = await this.transactionValidator.isFirstToday(userId);
    if (firstToday) {
      // First today.
      logger.info(`${userId} committing: is first today :)`);
      ...
      // Not first today.
      logger.info(`${userId} committing: is not first today :(`);
  ...
  _onCommitFailure(transaction, confirm) {
    ...
    logger.warn(`${userId} WTF: wrong confirm param: ${confirm}`);
