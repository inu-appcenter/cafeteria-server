...
class TransactionService {
  ...
  async validateDiscountTransaction({transaction, transactionToken}) {
    ...
    const basicTestResult = await this._testBasicThings(transaction, 'Validation');
    ...
    const tokenTestResult = await this._testTokenThing(transaction, transactionToken, 'Validation');
    ...
    return await this._onValidationSuccess(transaction);
  }
    ...
    const onFailAt = async (at) => {
      ...
      await this.transactionRepository.leaveTransactionHistory(
        new TransactionHistory({
          ...
          failedAt: at,
          message: await this._ruleSummary(at),
        }),
      );
    };
  ...
  async _onValidationSuccess(transaction) {
    await this._leaveHistory(transaction, 'Validation');
  ...
  async _onCommitSuccess(transaction, transactionToken, confirm) {
    ...
    await this._leaveHistory(transaction, 'Commit');
