...
class Injector {
  constructor() {
    ...
    this._temporalInstances = new Map();
  }
  ...
  resolve(classType) {
    ...
    if (!instance) {
      throw new Error(`No instance of '${this._safeName(classType)}'`);
    }
  ...
  _findInstance(classType) {
    const temporalInstance = this._temporalInstances.get(classType);
    if (temporalInstance) {
      this._temporalInstances.delete(classType);
  ...
  override(classType, instance) {
    this._instances.set(classType, instance);
  }

  overrideOnce(classType, instance) {
    this._temporalInstances.set(classType, instance);
  }
